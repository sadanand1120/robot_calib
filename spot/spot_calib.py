...
class SpotCameraCalibration:
    def __init__(self, resolution=1440, extrinsics_rel_filepath=None):
        self.resolution = resolution
        self.intrinsics_filepath = os.path.join(os.path.dirname(os.path.realpath(__file__)), f"params/cam_intrinsics_{self.resolution}.yaml")
        if extrinsics_rel_filepath is None:
            self.extrinsics_filepath = os.path.join(os.path.dirname(os.path.realpath(__file__)), "params/baselink_to_cam_extrinsics.yaml")
        else:
            self.extrinsics_filepath = os.path.join(os.path.dirname(os.path.realpath(__file__)), extrinsics_rel_filepath)
    ...
    def get_M_ext(self):
        ...
        T1 = SpotCameraCalibration.get_std_trans(cx=self.extrinsics_dict['T12']['T1']['X'] / 100,
                                                 cy=self.extrinsics_dict['T12']['T1']['Y'] / 100,
                                                 cz=self.extrinsics_dict['T12']['T1']['Z'] / 100)

        # Transforms from Intermediate frame to CCS
        T2 = SpotCameraCalibration.get_std_rot(axis=self.extrinsics_dict['T23']['R1']['axis'],
                                               alpha=np.deg2rad(self.extrinsics_dict['T23']['R1']['alpha']))
        T3 = np.array(self.extrinsics_dict['T23']['R2'])
        T4 = SpotCameraCalibration.get_std_rot(axis=self.extrinsics_dict['T23']['R3']['axis'],
                                               alpha=np.deg2rad(self.extrinsics_dict['T23']['R3']['alpha']))
        T5 = SpotCameraCalibration.get_std_rot(axis=self.extrinsics_dict['T23']['R4']['axis'],
    ...
    def projectWCStoCCS(self, wcs_coords):
        ...
        wcs_coords_4d = SpotCameraCalibration.get_homo_from_ordinary(wcs_coords)
        M_ext = self.get_M_ext()
        ccs_coords_4d = (M_ext @ wcs_coords_4d.T).T
        return SpotCameraCalibration.get_ordinary_from_homo(ccs_coords_4d)
    ...
    def projectCCStoPCS(self, ccs_coords, mode="skip"):
        ...
        safe_image_points_mask = SpotCameraCalibration.get_safe_projs(d, ccs_coords)
        ...
        image_points, pcs_mask = SpotCameraCalibration.to_image_fov_bounds(image_points, self.img_width, self.img_height, mode=mode)
    ...
    def projectCCStoWCS(self, ccs_coords):
        ...
        """
        ccs_coords = np.array(ccs_coords).astype(np.float64)
        ccs_coords_4d = SpotCameraCalibration.get_homo_from_ordinary(ccs_coords)
    ...
    def projectPCStoWCSground(self, pcs_coords, apply_dist=True, mode="skip"):
        ...
        undistorted_pcs_coords, pcs_mask = SpotCameraCalibration.to_image_fov_bounds(undistorted_pcs_coords, self.img_width, self.img_height, mode=mode)
        if apply_dist:
            pcs_coords = undistorted_pcs_coords
        pcs_coords_3d = SpotCameraCalibration.get_homo_from_ordinary(pcs_coords)
        M_ext = self.get_M_ext()
        M_ext_short = M_ext[:, [0, 1, 3]][:-1, :].reshape((3, 3))
        H = K @ M_ext_short  # 3x3
        H_inv = np.linalg.inv(H)
        wcs_coords_3d = (H_inv @ pcs_coords_3d.T).T
        wcs_coords = SpotCameraCalibration.get_ordinary_from_homo(wcs_coords_3d)
    ...
    def general_project_A_to_B(inp, AtoBmat):
        ...
        inp_4d = SpotCameraCalibration.get_homo_from_ordinary(inp)
        out_4d = (AtoBmat @ inp_4d.T).T
        return SpotCameraCalibration.get_ordinary_from_homo(out_4d)
...
if __name__ == "__main__":
    ...
    est_wcs_coords, _ = c.projectPCStoWCSground(gt_pcs_coords)

    _3dto2d_error = est_pcs_coords - gt_pcs_coords
    _2dto3d_error = (est_wcs_coords - gt_wcs_coords) * 100
    print("--------------------- 3D to 2D error (pixels):")
    SpotCameraCalibration.pretty_print(_3dto2d_error)
    print(np.mean(np.abs(_3dto2d_error)))
    print("-------------------------------------------------------")

    print("--------------------- 2D to 3D error (cms):")
    SpotCameraCalibration.pretty_print(_2dto3d_error)
    print(np.mean(np.abs(_2dto3d_error[:, :-1])))
    print("-------------------------------------------------------")
