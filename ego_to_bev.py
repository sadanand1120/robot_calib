...
try:
    ...
    pass
try:
    from jackal_calib.lidar_cam_calib import JackalLidarCamCalibration
except ImportError:
    pass
...
class EgoToBEV:
    def __init__(self):
        ...
        self.bev_lidar_cam = JackalLidarCamCalibration(ros_flag=False, cam_extrinsics_filepath="/home/dynamo/AMRL_Research/repos/synapse/config/bev_params/baselink_to_zed_left_extrinsics.yaml")

    def get_3d_bev(self, pil_img, pc_np, inpaint=True):
        """
        Uses the lidar pointcloud to calculate BEV image.
        pil_img: A x B PIL Image object (RGB)
        pc_np: N x ? numpy array of point cloud (x, y, z, whatever), this is corrected pointcloud
...
if __name__ == "__main__":
    ...
    pc_np = np.fromfile("depth.bin", dtype=np.float32).reshape((-1, 3))
    ego_to_bev = EgoToBEV()
    ...
    axs[1].imshow(ego_to_bev.get_3d_bev(raw_pil_img, pc_np))
    plt.show()