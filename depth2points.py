...
class D2P:
    def __init__(self):
        self.lcc = JackalLidarCamCalibration(ros_flag=False)
        self.DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.depth_obj = DepthAny2(device=self.DEVICE, model_input_size=518, max_depth=1, encoder='vitl', dataset='hypersim')
    ...
    def project_points_kinect_to_lidar(self, points_kinect):
        M_baselink_to_kinect = self.lcc.jackal_cam_calib.get_M_ext()
        M_baselink_to_lidar = self.lcc.get_M_ext()
        M_kinect_to_lidar = M_baselink_to_lidar @ np.linalg.inv(M_baselink_to_kinect)
        return self.lcc.jackal_cam_calib.general_project_A_to_B(points_kinect, M_kinect_to_lidar)
    
    @torch.inference_mode()
    def main(self, cv2_img, pc_np_xyz):
        """
        this pc_np_xyz is assumed to be corrected one
        """
        with torch.device(self.DEVICE):
            ...
            scaling_facs = ccs_zs / depths_rel
            max_d = float(np.median(scaling_facs))
            depth_arr = depth_rel_img * max_d
            kinect_points = self.depth2points(depth_arr, self.lcc.jackal_cam_calib.intrinsics_dict)
...
if __name__ == "__main__":
    ...
    lidar_points, depth_arr = d2p.main(cv2_img, pc_np[:, :3])
